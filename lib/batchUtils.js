...
const getSetSupportingDocumentsCompleteTransactions = async (
  ...
  return new Promise(async (resolve, reject) => {
    ...
    for (const [index, transactionData] of jsonData.entries()) {
      ...
      try {
        const { bountyId } = await loadGithubData(githubIssueUrl);
				...
				let userId;
        try {
          userId = await loadGithubDataUser(winnerGithubProfileUrl, 0);
        } catch (error) {
        ...
        const { payoutSchedule, payoutTokenAddress, tierWinners } = bounty;
				...
				const { login } = await loadGithubDataUserById(winnerGithubProfileUrl)
        ...
        supportingDocsBatchTransactionCopy.contractInputsValues._data = tier.toString();
