	...
	return new Promise(async (resolve, reject) => {
		...
		for (const transactionData of jsonData) {
			...
			try {
				...
				if (tier == null) {
					return reject(`User ${winnerGithubProfileUrl} for bounty ${githubIssueUrl} has no available tiers for ${tierAmount}.\ Tier winners are ${tierWinners}}. Payout schedule is ${payoutSchedule}`)
				}
	
				tiersClaimedPreviouslyInBatch.push(tier);
				
				const userId = await loadGithubDataUser(winnerGithubProfileUrl, tier);
