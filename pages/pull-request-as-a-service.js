  ...
  async function getBountyData(sortOrder, currentPagination, orderBy, cursor) {
    ...
    if (orderBy === 'tvl') {
      try {
        ...
        newBounties = prismaBounties.bountiesConnection.bounties.map((bounty) => {
          return {
            ...bounty,
            ...subgraphBounties.find((subgraphBounty) => subgraphBounty.bountyAddress === bounty.address.toLowerCase()),
          };
        });
      } catch (err) {
