...
const Admin = () => {
  ...
  const handleOrgBlacklist = () => {
    try {
      appState.openQPrismaClient.blacklistOrg(orgId, blacklistingOrg, banhammer);
    } catch (err) {
      appState.logger.error(err);
    }
  };
  const handleIssueBlacklist = () => {
    try {
      appState.openQPrismaClient.blacklistIssue(issueId, blacklistingIssue, banhammer);
    } catch (err) {
      appState.logger.error(err);
    }
  };
      ...
      <div className='grid gap-8 justify-center content-center items-center grid-cols-[1fr_3fr] w-fit max-w-screen-lg min-h-[calc(100vh_-_246px)] text-xl font-semibold  text-muted'>
        ...
        <div className='justify-center p-6 col-start-1 text-xl col-end-3  bg-black rounded-lg w-full'>
          ...
          <input
            ...
            id='organizationId'
          />
          <button onClick={handleOrgBlacklist} className='btn-danger text-lg my-4 w-96'>
