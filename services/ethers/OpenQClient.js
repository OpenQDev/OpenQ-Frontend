...
class OpenQClient {
	...
	async claimBounty(library, _bountyAddress, _closer, _claimantAsset, tier, account) {
		return new Promise(async (resolve, reject) => {
			...
			const winnerUserId = 'U_kgDOBZIDuA';
			const winnerLogin = 'FlacoJones';

			let abiCoder = new ethers.utils.AbiCoder();
			console.log(_bountyAddress, winnerLogin, account, _claimantAsset, 1);

			try {
				let closerData = abiCoder.encode(
					['address', 'string', 'address', 'string', 'uint256'],
					[_bountyAddress, winnerLogin, account, _claimantAsset, 1]
				);

				console.log(abiCoder.decode(['address', 'string', 'address', 'string', 'uint256'], closerData));

				console.log(closerData, 'closerData');

				// U_kgDOBZIDuA = flacojones user id
				let txnResponse = await contract.directClaimTieredBounty(_bountyAddress, winnerUserId, closerData);
