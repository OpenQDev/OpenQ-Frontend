  ...
  async checkAuth() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const { isAuthenticated, avatarUrl, login, githubId, email } = response.data;
        resolve({ type: 'UPDATE_IS_AUTHENTICATED', payload: { isAuthenticated, avatarUrl, login, githubId, email } });
      } catch (error) {
        reject(error);
      }
    });
