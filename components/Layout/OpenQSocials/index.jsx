...
const OpenQSocials = ({ isWhite }) => {
  return (
    <div className='flex bottom-24 left-16 gap-2'>
      <a href={'https://twitter.com/openqlabs'} target={'_blank'} rel='noopener noreferrer'>
        <Twitter isWhite={isWhite} />
      </a>
      <a href={'https://github.com/OpenQDev/'} target={'_blank'} rel='noopener noreferrer'>
        <svg xmlns='http://www.w3.org/2000/svg' className={`w-6 h-6`} viewBox='0 0 24 24' width='48px' height='48px'>
          <path
            className={`fill-black ${isWhite ? 'fill-gray-800' : 'fill-white'}`}
      ...
      <a href={'https://discord.gg/uUUdywtBqh'} target={'_blank'} rel='noopener noreferrer'>
        <Discord isWhite={isWhite} />
      </a>
    </div>
  );
};
