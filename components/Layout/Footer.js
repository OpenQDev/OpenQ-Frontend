					...
					<div className='relative h-6'>
						<div className={`relative w-36 ${open && 'bottom-14 p-2 bg-nav-bg rounded-sm border border-web-gray'}`}>
							{open ?
								<div ref={modal}>
									<button onClick={
										()=>setToggle(1)} value ={1} className='block'>
									...
									<button onClick={()=>setToggle(3)} value={3} className='block'>
										<CopyAddressToClipboard clipping={[5, 38]} data={process.env.NEXT_PUBLIC_CLAIM_MANAGER_PROXY_ADDRESS} styles="pt-0 w-64" />
									</button>
								</div>:
								...
								toggle===1 ?
									...
									toggle===2 ?
										...
										<button value={3} className='block'>
											<CopyAddressToClipboard clipping={[5, 38]} data={ethers.utils.getAddress(process.env.NEXT_PUBLIC_CLAIM_MANAGER_PROXY_ADDRESS||'')} styles="pt-0 w-64" />
										</button>
