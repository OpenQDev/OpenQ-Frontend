...
const LinkDropdown = ({  items, styles }) => {
	...
	// Render
	return (
		<div ref={ref} open={true}  className={`max-h-0 ${styles} px-1 hidden group-focus-within:block group-focus:block text-sm text-muted `}>
			
			<div className='w-full'>
				{items.length >0 ? <ul className='z-20 overflow-hidden relative bg-subtle w-full -mt-1 mb-4 shadow-[rgb(1,_4,_9)_0px_8px_24px_0px] border-web-gray border-x border-b rounded-b-sm'>
					{items.map(item=>
						...
						<li  key = {item.url}>
							...
							<Link href={item.url}>
								<a className='cursor-pointer flex  w-full gap-4 hover:bg-link-colour text-white py-2 px-4 border-t border-web-gray'>
									{
										item.isIssue ?
											<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width="16" height="16" className='fill-primary relative top-0.5'>
				...
				Nothing matches you{'\''}re search.</div>
				}
			</div>
		</div>);
};
