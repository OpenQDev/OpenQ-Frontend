...
const ToolTip = (props)=>{
	...
	return (
		<div className={`relative group rounded-full ${props.outerStyles}`} onMouseEnter={setToolTip}>
			{props.children}
			<div style={{left: x, top: y}} className={`flex justify-center absolute hidden z-10 group-hover:block justify-items-center w-full h-3 pt-0.5 ${mobileX === x ? 'w-72' : styles}`}>
