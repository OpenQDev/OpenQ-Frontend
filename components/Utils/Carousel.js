...
const Carousel = (props) => {
	...
	// render
	return (
		<div onMouseUp={()=>setIsScrolling()} className={`relative ${isScrollable && 'xl:-left-10 xl:-right-10  xl:w-[calc(100%+80px)]'} justify-center self-center w-full flex ${styles}`}>
			{isScrollable &&	<button className='sr-only xl:not-sr-only' data-testid="scroll carousel left" onMouseDown={()=>setIsScrolling('left')}>	
				<svg className='hover:fill-web-gray fill-dark-mode stroke-web-gray' width="44" height="36" viewBox="0 0 44 66" xmlns="http://www.w3.org/2000/svg">
			...
			{isScrollable&& <button className='sr-only xl:not-sr-only' data-testid="scroll carousel right" onMouseDown={()=>setIsScrolling('right')} >
				<svg className='hover:fill-web-gray fill-dark-mode stroke-web-gray'  width="44" height="36" viewBox="0 0 44 66" xmlns="http://www.w3.org/2000/svg">
