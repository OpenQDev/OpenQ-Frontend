...
const Carousel = ({styles, children}) => {
	...
	return (
		<div onMouseUp={()=>setIsScrolling()} className={`relative ${isScrollable && 'lg:-left-10 lg:-right-10  lg:w-[calc(100%+80px)]'} justify-center self-center w-full flex ${styles}`}>
			{isScrollable &&	<button className='sr-only lg:not-sr-only' onMouseDown={()=>setIsScrolling('left')}>	
			...
			<div ref={scroller} className='flex justify-center gap-2 lg:max-w-[100%] lg:w-[900px] overflow-x-auto pt-4 pb-4 px justify-items-center'>
