    ...
    async (iframe) => {
      if (iframe && tokenValues && appState.tokenClient) {
        ...
        const tableHeaders = await Promise.all(
          bounty.bountyTokenBalances.map(async (token) => {
        ...
        doc.autoTable({
          ...
          body: [
            ...
            [
              ...
              ...tableData.map(() => {
                return { styles: { lineWidth: 0 }, content: '' };
              }),
    ...
    <div>
      ...
      <div className='grid xl:grid-cols-[1fr_1fr] gap-x-4 justify-center justify-items-center px-32 py-8'>
        <form className=' h-min'>
          <div className='grid md:grid-cols-[1fr_1fr] gap-4 align-content-start pb-4'>
            <input
              onChange={(e) => {
                setYourName(e.target.value);
              }}
              className='bg-dark-mode border border-web-gray text-xl p-4 h-min rounded-lg focus:outline-none'
              type='text'
              placeholder='Name'
            ></input>
            <input
          ...
          <ToolTipNew
            toolTipText='Cannot generate invoice if there are no deposits.'
            hideToolTip={tokenValues}
            outerStyles={'bottom-5'}
          >
            <button
              onClick={handleSubmit}
              disabled={!tokenValues}
              className={` ${!tokenValues ? 'confirm-btn-disabled' : 'confirm-btn'} col-span-2 my-4`}
            >
              Generate
            </button>
          </ToolTipNew>
