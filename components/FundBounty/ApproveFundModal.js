							...
							approveTransferState === SUCCESS ?
								<>
									<div className="text-md gap-4 py-6 px-4 grid grid-cols-[1fr_1fr] w-full justify-between">
										...
										<div className='flex justify-between w-[120px]'><Image width={24} className="inline" height={24} src={token.path || token.URI} /><span>{volume} {token.symbol}</span></div>
										<span>To</span>
								...
								<>
									<div className="text-md gap-4 py-6 px-4 grid grid-cols-[1fr_1fr] w-full justify-between">
										...
										<div className='flex justify-between w-[120px]'><Image width={24} className="inline" height={24} src={token.path || token.URI} /><span className='truncate'>{volume} {token.symbol}</span></div>
										<span>To</span>
										<CopyAddressToClipboard data={bountyAddress} clipping={[5, 39]} />
										<span>For</span>
										<Link href={bounty.url}><a target="_blank" rel="noopener noreferrer" className='underline'>{bounty.title}</a></Link>
