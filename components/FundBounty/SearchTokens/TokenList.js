...
const TokenList = ({
  ...
  if (lists.openq) {
    const openqTokens = openqDefaultTokens.filter(
      (token) =>
        !fetchedTokens.some((displayToken) => displayToken.address.toLowerCase() === token.address.toLowerCase())
    );
  ...
  if (lists.polygon) {
    fetchedTokens = fetchedTokens.concat(
      polygonTokens.filter(
        (token) => !fetchedTokens.some((displayToken) => displayToken.address.toLowerCase() === token.address)
      )
    );
  }
