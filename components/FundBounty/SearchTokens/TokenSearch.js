	...
	return (
		<div>
			<div onClick={handleOutsideClick} className="justify-center font-mont items-center flex overflow-x-hidden overflow-y-auto fixed inset-0 z-50 outline-none focus:outline-none">
				<div className="w-5/6 max-w-md"> <div onClick={(e) => e.stopPropagation()} className="flex justify-left border border-web-gray pl-8 pr-8 pt-5 pb-3 rounded-lg shadow-lg flex flex-col w-full bg-dark-mode outline-none focus:outline-none">
					{showListManager ?
						<div className='h-[30rem]'>
							<div className="flex items-center justify-between border-solid rounded-t pt-2 pb-4">
								...
								<button className='text-3xl hover:text-tinted'	onClick={() => setShowTokenSearch(false)}>×</button>
							</div>
							...
							<div className="flex flex-col justify-items-center gap-2 justify-end p-6 border-t border-solid rounded-b">
								
							</div>
						</div>:
						<ManageTokenList setLists={setLists} setCustomTokens={setCustomTokens} customTokens={customTokens} lists={lists}/>
					}
					<button className='confirm-btn' onClick={(e)=>{setShowListManager(()=>!showListManager);
						e.stopPropagation();
					}}>{showListManager? 'Manage token lists' : 'Back' }</button></div>
				</div>
			</div>
